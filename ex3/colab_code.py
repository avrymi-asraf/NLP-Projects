...
def create_or_load_slim_w2v(words_list, cache_w2v=False):
    ...
    """
    w2v_path = "NLP-Projects/w2v_dict.pkl"
...
def train_log_linear_with_one_hot(device="cpu", use_sub_phrases=True) -> pd.DataFrame:
    ...
    data_manager = DataManager(
        ...
        data_type=ONEHOT_AVERAGE,
        dataset_path="NLP-Projects/ex3/stanfordSentimentTreebank",
...
def make_graph(run_data, session: str):
    ...
    plt.savefig(f'{session}_loss.png')
    ...
    plt.grid(True)
    plt.xticks(np.arange(1, len(run_data.index) + 1, 1))
    plt.savefig(f'{session}_Acc.png')
...
if __name__ == "__main__":
    ...
    # record_data_w2v_average = pd.read_csv("ex3\\record_data_w2v.csv")
    ...
    # record_data_lstm = pd.read_csv("ex3\\record_data_lstm.csv")
